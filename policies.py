...
class Policy:
    ...
    def build_graph(self):
        ...
        out = U.dense(self.observation, 10, 'layer1', weight_init=tf.contrib.layers.xavier_initializer(), bias=True,activation=tf.nn.tanh, summary=self.summary)
        out = U.dense(out, 10, 'layer2', weight_init=tf.contrib.layers.xavier_initializer(), bias=True, activation=tf.nn.tanh, summary=self.summary)
...
class GoPolicy(Policy):
    ...
    def build_graph(self):
        ...
        conv3 = U.conv(conv2, name='conv3', filters=44, kernel_size=5, strides=1,use_bias=True, activation=activation, padding="valid",summary=self.summary)
